...
function reSort(input, symbolData, arraySymbol, symbol) {
...
function combine(tempSymbol, symbolData) {
...
module.exports = function(data, symbol) {
  ...
  for(let itemId in data) {
    ...
    let symbolData = {};
    let arraySymbol = [];
    reSort(item, symbolData, arraySymbol, symbol.unity); 

    for(let symbolId in arraySymbol) {
      let outputSymbol = { symbol: { lon: "", lat: "" }, data: {} };
      let tempSymbol = arraySymbol[symbolId];
      combine(tempSymbol, symbolData);
      findSymbol(symbolData, outputSymbol, symbol);
